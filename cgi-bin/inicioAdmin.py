...
cursor = conn.cursor()
#Query de comprobacion de la existencia de la sesion
if form.getvalue("user") is not None:
	sql = "SELECT * FROM admin where user='"+form.getvalue("user")+"'"
	cursor.execute(sql)
	resultado = cursor.fetchall()
	for row in resultado:
		sesionActiva = row.islogged
...
if sesionActiva==False:
	...
	if form.getvalue("user") is not None:
		...
		if len(arrayUser)>0:
			sesionActiva = True
			sql = "UPDATE admin SET islogged=True where user='"+form.getvalue('user')+"'"
			cursor.execute(sql)
			conn.commit()
...
if sesionActiva==True:
	...
	if not rows: #Login incorrecto
		...
		print "<h3 class='card-title'>Bienvenido: "+ form.getvalue("user")+"</h3>"
		...
		print "</tbody>"
		print "</table>"
		if habilitarBotonCancelar == True:
			print "<div class='col-lg-3'>"  
			...
			print "<div class='col-lg-3'>"  
			print "<input class='btn btn-info btn-block waves-effect waves-light' type='submit' value='Cancelar Turnos' formaction='/cgi-bin/cancelarTurno.py' disabled>"
			print "</div>" # class='col-lg-3'
		print "</div>" # id=divModificar
		print "</div>" # class='card-body'
		print "</div>" # class='col-lg-10'
